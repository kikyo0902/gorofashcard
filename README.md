...
<body>
  <div class="wrap">
    <header>
      <div>
        <h1 data-i18n="appTitle">Flash Cards — Học tiếng Nhật (Deck/Danh sách)</h1>
        <div class="sub">
          <span data-i18n="headerTip">Bấm thẻ để lật</span>
          • <span class="kbd">Space</span> <span data-i18n="kbdFlip">lật</span>
          • <span class="kbd">←</span>/<span class="kbd">→</span> <span data-i18n="kbdNav">chuyển thẻ</span>
        </div>
      </div>
      <div class="controls">
        ...
        <button id="shuffleBtn" data-i18n="shuffle">Ngẫu nhiên</button>
        <button class="primary" id="resetProgress" data-i18n="resetProgress">Reset tiến độ</button>
      </div>
    </header>
    ...
    <div class="grid">
      ...
      <section class="panel">
        <div class="controls" style="justify-content:space-between">
          <div class="controls">
            <button id="prevBtn" data-i18n="prev">← Trước</button>
            <button id="nextBtn" data-i18n="next">Sau →</button>
            <button id="flipBtn" data-i18n="flip">Lật</button>
          </div>
          <div class="controls">
            <button class="bad" id="markUnknown" data-i18n="unknown">Chưa nhớ</button>
            <button class="good" id="markKnown" data-i18n="known">Đã nhớ</button>
          </div>
        </div>
            ...
            <div class="cardInner">
              <div class="face front">
                <div class="big">
                  ...
                  <div class="kana" id="frontSub" data-i18n="tapToFlip">Bấm để lật</div>
                  <div class="tagRow">
              ...
              <div class="face back">
                <div class="big">
                  ...
                  <div class="hint" id="example" data-i18n="examplePrefix">Ví dụ: —</div>
                </div>
              </div>
            </div>
          </div>
          <div class="hint" data-i18n="rightTip">Tip: Tạo deck và gán từ vào deck ở panel bên phải →</div>
        </div>
      </section>
      ...
      <aside class="panel">
        <div class="rowBetween">
          <div>
            <h2 style="margin:0 0 6px;font-size:16px" data-i18n="deckTitle">Deck (Danh sách)</h2>
            <div class="sub" data-i18n="deckSubtitle">Tạo deck riêng: N5, N4, Công việc, Hội thoại…</div>
          </div>
        </div>

        <div class="controls" style="margin-top:8px">
          <input id="newDeckName" data-i18n-placeholder="newDeckPlaceholder" placeholder="Tên deck mới (vd: Công việc)" />
          <button class="primary" id="createDeckBtn" data-i18n="createDeck">+ Tạo deck</button>
          <button class="bad" id="deleteDeckBtn" data-i18n="deleteDeck" title="Xóa deck đang chọn (không xóa thẻ)">Xóa deck</button>
        </div>
        ...
        <div style="margin-top:12px">
          <div class="rowBetween">
            <div>
              <h2 style="margin:0;font-size:16px" data-i18n="deckManageTitle">Quản lý deck</h2>
              <div class="sub" data-i18n="deckManageSubtitle">Thống kê % đã nhớ theo từng deck.</div>
            </div>
          </div>
        ...
        <h2 style="margin:0 0 6px;font-size:16px" data-i18n="addCardTitle">Thêm Flash Card</h2>
        <div class="sub" data-i18n="addCardSubtitle">Khi thêm thẻ, chọn deck muốn lưu.</div>

        <div class="formRow">
          <select id="deckSelect" class="full"></select>

          <input id="jp" data-i18n-placeholder="phJp" placeholder="日本語 (Kanji/Kana) — ví dụ: 勉強" />
          <input id="kana" data-i18n-placeholder="phKana" placeholder="Kana — ví dụ: べんきょう" />
          <input id="vn" class="full" data-i18n-placeholder="phVn" placeholder="Nghĩa tiếng Việt — ví dụ: học tập" />
          <input id="cat" data-i18n-placeholder="phCat" placeholder="Chủ đề — ví dụ: công việc" />
          <input id="level" data-i18n-placeholder="phLevel" placeholder="JLPT — ví dụ: N4" />
          <textarea id="ex" class="full" data-i18n-placeholder="phEx" placeholder="Ví dụ câu (tuỳ chọn) — ví dụ: 毎日日本語を勉強します。"></textarea>
        </div>

        <div class="controls" style="margin-top:10px">
          <button class="primary" id="addBtn" data-i18n="addCard">+ Thêm thẻ</button>
          <button id="importBtn" data-i18n="importSample">Import mẫu</button>
          <button id="clearAllBtn" data-i18n="clearAll">Xóa tất cả</button>
        </div>
        ...
        <h2 style="margin:0 0 6px;font-size:16px" data-i18n="qrTitle">QR mở web</h2>
        <div class="sub" data-i18n="qrSubtitle">Quét để mở trang này trên điện thoại.</div>
        <div id="qrBox" style="margin-top:10px"></div>
        <button id="regenQR" style="margin-top:10px" data-i18n="qrRegen">Tạo lại QR</button>
      </aside>
    </div>
  </div>
  ...
  <script>
    ...
    const I18N = {
      vi: {
        appTitle: "Flash Cards — Học tiếng Nhật (Deck/Danh sách)",
        headerTip: "Bấm thẻ để lật",
        kbdFlip: "lật",
        kbdNav: "chuyển thẻ",
        shuffle: "Ngẫu nhiên",
        ...
        examplePrefix: "Ví dụ: ",
        rightTip: "Tip: Tạo deck và gán từ vào deck ở panel bên phải →",

        deckTitle: "Deck (Danh sách)",
        deckSubtitle: "Tạo deck riêng: N5, N4, Công việc, Hội thoại…",
        createDeck: "+ Tạo deck",
        deleteDeck: "Xóa deck",
        newDeckPlaceholder: "Tên deck mới (vd: Công việc)",

        deckManageTitle: "Quản lý deck",
        deckManageSubtitle: "Thống kê % đã nhớ theo từng deck.",
        tblDeck: "Deck",
        tblProgress: "Tiến độ",
        tblPercent: "%",
        tblAction: "Hành động",
        btnStudyDeck: "Học deck",
        btnRename: "Đổi tên",
        btnDelete: "Xóa",
        deckIdLabel: "id",
        progressLabel: "đã nhớ / tổng",

        addCardTitle: "Thêm Flash Card",
        addCardSubtitle: "Khi thêm thẻ, chọn deck muốn lưu.",
        addCard: "+ Thêm thẻ",
        importSample: "Import mẫu",
        clearAll: "Xóa tất cả",

        phJp: "日本語 (Kanji/Kana) — ví dụ: 勉強",
        phKana: "Kana — ví dụ: べんきょう",
        phVn: "Nghĩa tiếng Việt — ví dụ: học tập",
        phCat: "Chủ đề — ví dụ: công việc",
        phLevel: "JLPT — ví dụ: N4",
        phEx: "Ví dụ câu (tuỳ chọn) — ví dụ: 毎日日本語を勉強します。",
        ...
        deckAll: "Tất cả (All)",
        deckDefault: "Mặc định",

        statsTotal: "Tổng thẻ",
        statsKnownAll: "Đã nhớ (tổng)",
        statsShowing: "Hiển thị",
        statsKnownShowing: "Đã nhớ (hiển thị)",
        statsPos: "Vị trí",

        emptyTitle: "Chưa có thẻ",
        emptySubtitle: "Hãy thêm thẻ hoặc Import mẫu",

        alertNeedDeckName: "Nhập tên deck trước nhé.",
        alertDeckExists: "Deck này đã tồn tại.",
        alertCannotDeleteAll: "Không thể xóa 'Tất cả (All)'.",
        alertCannotDeleteDefault: "Không thể xóa deck 'Mặc định'.",
        confirmDeleteDeck: 'Xóa deck "{name}"?\n(Thẻ trong deck sẽ chuyển về "Mặc định")',
        confirmDeleteCard: "Xóa thẻ này?",
        confirmAddSample: "Thêm bộ mẫu vào danh sách hiện tại?",
        confirmClearAll: "Xóa toàn bộ thẻ, deck và tiến độ?",
        alertNeedJpVn: "Hãy nhập ít nhất: 日本語 và Nghĩa tiếng Việt.",
        promptRenameDeck: "Tên mới cho deck:",
        alertDeckNameDuplicate: "Tên deck bị trùng.",

        listEmpty: "Chưa có dữ liệu. Bấm Import mẫu hoặc thêm thẻ mới.",

        qrTitle: "QR mở web",
        qrSubtitle: "Quét để mở trang này trên điện thoại.",
        qrRegen: "Tạo lại QR",
        qrNoUrl: "QR cần URL (không phải file:///). Hãy up web lên GitHub Pages/Netlify để quét mở trên điện thoại."
      },
      ja: {
        appTitle: "フラッシュカード — 日本語学習（デッキ管理）",
        headerTip: "カードをタップして裏返す",
        kbdFlip: "めくる",
        kbdNav: "カード移動",
        shuffle: "シャッフル",
        ...
        tapToFlip: "タップしてめくる",
        examplePrefix: "例文：",
        rightTip: "Tip：右側でデッキ作成＆単語をデッキに追加 →",

        deckTitle: "デッキ（リスト）",
        deckSubtitle: "N5 / N4 / 仕事 / 会話など、自由にデッキを作成できます。",
        createDeck: "+ デッキ作成",
        deleteDeck: "デッキ削除",
        newDeckPlaceholder: "新しいデッキ名（例：仕事）",

        deckManageTitle: "デッキ管理",
        deckManageSubtitle: "デッキ別の習得率（％）を表示します。",
        tblDeck: "デッキ",
        tblProgress: "進捗",
        tblPercent: "%",
        tblAction: "操作",
        btnStudyDeck: "このデッキを学習",
        btnRename: "名前変更",
        btnDelete: "削除",
        deckIdLabel: "id",
        progressLabel: "習得 / 合計",
        ...
        addCardTitle: "カード追加",
        addCardSubtitle: "追加時に保存先デッキを選択します。",
        addCard: "+ 追加",
        importSample: "サンプル追加",
        clearAll: "全削除",

        phJp: "日本語（漢字/かな）例：勉強",
        phKana: "読み（かな）例：べんきょう",
        phVn: "意味（ベトナム語）例：学習",
        phCat: "カテゴリー 例：仕事",
        phLevel: "JLPT 例：N4",
        phEx: "例文（任意）例：毎日日本語を勉強します。",
        ...
        statsKnownAll: "習得（総）",
        statsShowing: "表示中",
        statsKnownShowing: "習得（表示）",
        statsPos: "位置",

        emptyTitle: "カードがありません",
        emptySubtitle: "カードを追加するか、サンプルを入れてください。",

        alertNeedDeckName: "デッキ名を入力してください。",
        alertDeckExists: "そのデッキ名は既に存在します。",
        alertCannotDeleteAll: "「すべて（All）」は削除できません。",
        alertCannotDeleteDefault: "「デフォルト」は削除できません。",
        confirmDeleteDeck: 'デッキ「{name}」を削除しますか？\n（カードは「デフォルト」に移動します）',
        confirmDeleteCard: "このカードを削除しますか？",
        confirmAddSample: "サンプルを現在のリストに追加しますか？",
        confirmClearAll: "カード・デッキ・進捗をすべて削除しますか？",
        alertNeedJpVn: "最低限「日本語」と「意味」を入力してください。",
        promptRenameDeck: "新しいデッキ名：",
        alertDeckNameDuplicate: "デッキ名が重複しています。",

        listEmpty: "データがありません。「サンプル追加」または新規追加してください。",

        qrTitle: "QRで開く",
        qrSubtitle: "スマホで読み取って開きます。",
        qrRegen: "QRを再生成",
        qrNoUrl: "QRはURLが必要です（file:/// は不可）。GitHub Pages / Netlify に公開するとスマホで開けます。"
      }
    };
    ...
    if(!decks){
      ...
      if(!decks.find(d=>d.id==="default")){
        decks.push({id:"default", name:"Mặc định"});
        saveJSON(KEY_DECKS, decks);
      }
    }
    ...
    function renderEmpty(){
      ...
      backVn.textContent = "—";
      example.textContent = t("examplePrefix") + "—";
    ...
    function renderCard(){
      ...
      $("frontSub").textContent = known ? "✅ " + t("known") : "🟡 " + t("unknown");

      backText.textContent = c.jp || "—";
      backKana.textContent = c.kana ? `(${c.kana})` : "—";
      backVn.textContent = c.vn || "—";
      example.textContent = t("examplePrefix") + (c.ex || "—");
      ...
      listCards.forEach((c)=>{
        ...
        div.innerHTML = `
          <div>
            ...
            <div class="sub">📚 ${deckNameById(c.deckId || "default")}${(c.cat? " • #"+c.cat : "")}${c.level ? " • "+c.level : ""}${known ? " • ✅ "+t("known") : ""}</div>
          </div>
          <div class="itemBtns">
            <button data-go="${c.id}">${currentLang==="ja" ? "学習" : "Học"}</button>
            <button data-del="${c.id}" class="bad">${t("btnDelete") || (currentLang==="ja" ? "削除" : "Xóa")}</button>
          </div>
        `;
    ...
    function renderDeckStats(){
      ...
      if(s.length === 0){
        deckStatsWrap.innerHTML = `<div class="hint">—</div>`;
        return;
      }
    ...
    function renderQR(){
      ...
      // file:/// -> QR quét thường không mở được trên mobile
...
</html>    button.primary{background:rgba(124,92,255,.95);border-color:rgba(124,92,255,.6)}
    button.good{background:rgba(53,208,127,.95);border-color:rgba(53,208,127,.6);color:#07130c}
    button.bad{background:rgba(255,92,122,.95);border-color:rgba(255,92,122,.6)}
    button:disabled{opacity:.5;cursor:not-allowed}
    .stat{display:flex;gap:12px;flex-wrap:wrap;margin-top:10px;color:var(--muted);font-size:13px}
